...
rule query_ied_data:
    message: "Extract queries from the ied dataset (in .accdb format). This rule can only be executed on Windows machines."
    ...
    output:
        ...
        ied_match_facil_insp_inst_insp = "build/data/industrial-emissions/ied-data/ied-match-facil-insp-inst_insp.csv"
    conda: "../envs/query-msaccess.yaml"
    script: "../scripts/industrial-emissions/query_ied_data.py"


rule manipulate_ied_etsidentifiers:
    message: "Manipulate IED_ETSIdentifiers in preparation for matching of datasets. Script has to be adapted when using IED data from other years than 2020."
    input: rules.query_ied_data.output.ied_etsidentifiers # When importing these from a windows machine, make sure that modification time stemps are more recent than those of unzip_ied_data's output
    params: reportingYear_IED = 2020 # ASSUME ratio emissions of sector j in unit i / national emissions of sector j constant over years. Script has to be adapted when using IED data from other years than 2020.
...
rule preprocess_emission_data:
    message: "Preprocess ETS data. Preprocess IED data. Identify plants that occur in both datasets (matching). Join both datasets. Convert plants' activity codes to EUROSTAT format."
    input:
    ...
    params:
        reportingYear_ETS = 2020, # ASSUME ratio (emissions of sector j in unit i / national emissions of sector j) constant over years. Reporting year for the IED dataset is set in rule manipulate_ied_etsidentifiers
