...
def electricity_demand(path_to_units, path_to_electricity_load, path_to_result):
    ...
    pd.concat(
        [unit_time_series(unit, national_load) for _, unit in units.iterrows()],
        axis=1
    ).to_csv(path_to_result)


def unit_time_series(unit, national_load):
