...
def biofuel_potential(
    ...
    national_potentials = pd.concat(
        [
            ...
            .rename(index=utils.convert_country_code)
    ...
    if (len(units.index) == 1) and (
        ...
        national_potentials = pd.DataFrame(
            index=["EUR"], data=national_potentials.sum(axis=0).to_dict()
        )
    ...
    weighted_cost = (
        (costs * national_potentials / national_potentials.sum().sum()).sum().sum()
    )
