...
def get_heat_demand(
    ...
    path_to_carrier_names, heat_tech_params,
    path_to_electricity_consumption, path_to_output
):
    # TODO possibly merge this script with transport
...
def get_energy_balances(energy_balance, carrier_names):
    energy_balance_df = util.read_tdf(energy_balance)
    ...
    for balance_code, cat_code in balance_codes.items():
        ...
        assert _df.index.get_level_values('unit').unique().tolist() == ['TJ'], "There are other units than TJ in the energy balance data. This is not expected."

        _df = (
            _df.xs('TJ', level='unit')
            .apply(util.tj_to_twh)  # TJ -> TWh
            .unstack('year')
            .rename(index=carrier_names_df[f'{balance_code}_carrier_name'].dropna().to_dict(), level='carrier_code') 
            .groupby(by=['carrier_code', 'country'], level=['carrier_code', 'country'])
            .sum()
            .loc[ list(set(carrier_names_df[f'{balance_code}_carrier_name'].dropna())) ] # remove carriers that are not relevant
...
def get_household_energy_consumption(
    hh_end_use, ch_end_use, carrier_names
):
    """
    ...
    assert check_units_removed(hh_end_use_df, carrier_names_df), "Check that you can slice by 'TJ' only, some other units in the hh_end_use data might be relevant."
    ...
    not_countries = [c for c in hh_end_use_df.index.get_level_values('country_code').unique() if len(c) > 2] + ["XK"]
    hh_end_use_df = hh_end_use_df.drop(axis=0, level='country_code', labels=not_countries)
    country_codes_ = {c: utils.convert_country_code(c) for c in hh_end_use_df.index.get_level_values('country_code')}

    # Add missing renewables data to
    # rename index labels to be more readable

    hh_end_use_df = hh_end_use_df.groupby(
        [END_USE_CAT_NAMES, carrier_names_df['hh_carrier_name'].dropna().to_dict(), country_codes_],
...
def check_units_removed(df, carrier_names_df):

    not_countries = [c for c in df.index.get_level_values('country_code').unique() if len(c) > 2] + ["XK"]
    df = (  # first re-organise df
        df
        .stack('year')
        .unstack('unit')
        .loc[idx[END_USE_CAT_NAMES.keys(), carrier_names_df['hh_carrier_name'].dropna().index, :, :], :]
        .drop(axis=0, level='country_code', labels=not_countries)
    )
...
def ch_hh_consumption(ch_end_use):
    ...
    return ch_hh_end_use_df.apply(util.pj_to_twh)  # PJ -> TWh
...
def map_jrc_to_eurostat(energy_balance, jrc_end_use_df):
    jrc_end_use_df = (
        ...
        .apply(util.ktoe_to_twh)  # kTOE -> TWh
    )
    jrc_end_use_percent = (
        jrc_end_use_df
...
def get_annual_electricity_consumption(annual_consumption, energy_balance_dfs):
    ...
    electricity_consumption = (
        electricity_consumption
        .rename(lambda x: f'{x}_historically_electrified', level='end_use', axis=1)
    )
