...
def ch_transport_energy_balance(path_to_ch_energy_balance_excel, path_to_output):
    ...
    carrier_name_func = (
        lambda x: df.columns.to_frame()
                  .iloc[:, x]
                  .str.translate(utils.remove_digits())  # Footnote labels lead to some strings randomly ending in numbers; we remove them here
    ...
    da.expand_dims(country_code=["CHE"]).assign_attrs(unit="twh").to_netcdf(path_to_output)
