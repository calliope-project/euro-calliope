...
def ch_energy_balance(path_to_ch_energy_balance_excel: str, path_to_output: str):
    """
    Process Swiss data into an xarray dataset that matches the structure of the annual energy balances.
    """
...
def get_ch_energy_balance_excel_sheet(
    path_to_excel: str, sheet: str, skipfooter: int, cat_code: str
) -> xr.DataArray:
    """
    ...
    Args:
        path_to_excel (str): Path to CH annual energy balance Excel.
        sheet (str): sheet name in the Excel.
        skipfooter (int): number of footer rows to ignore which correspond to footnotes in the sheet.
        cat_code (str): sector (category) for which the sheet has data, corresponding to eurostat sectors.

    Returns:
        xr.DataArray: Processed array.
    """
    ...
    df = (
        ...
        .drop("davon Heizöl extra-leicht", level=1, axis=1)
    )
    df.columns = (
