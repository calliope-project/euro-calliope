...
def ch_building_energy_consumption_dataset(path_to_ch_end_use_excel, path_to_output):
    ...
    commercial_consumption = ch_non_hh_consumption(path_to_ch_end_use_excel, household_consumption)
...
def ch_non_hh_consumption(path_to_ch_end_use_excel, household_consumption):
    ...
    commercial_sheet_kwargs = {"translation": CH_HH_END_USE_TRANSLATION, "index_name": "end_use"}
    ...
    commercial_electricity_consumption = commercial_electricity_consumption.expand_dims(carrier_name=["electricity"])
    ...
    relevant_household_consumption = household_consumption.sel(end_use=commercial_fuel_consumption.end_use).dropna("carrier_name")

    household_consumption_shares = (
        relevant_household_consumption / relevant_household_consumption.sum("carrier_name")
    )
    ...
    commercial_energy_consumption = utils.merge_da(
        [commercial_electricity_consumption, commercial_fuel_consumption_assigned_to_end_uses]
    )
...
def ch_hh_consumption(path_to_ch_end_use_excel):
    ...
    Switzerland data isn't in Eurostat, so we get it from their govt. stats directly
    """
    household_sheet_kwargs = {"translation": CH_ENERGY_CARRIER_TRANSLATION, "index_name": "carrier_name"}
...
def get_ch_sheet(path_to_excel, sheet, skipfooter, index_name, translation=None):
    ...
    df = df.drop(2019, axis=1, errors="ignore")  # strip out the year 2019 in case it has appeared
