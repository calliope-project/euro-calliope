...
def construct_links(
    ...
    tyndp_scenarios = pd.read_excel(
        path_to_entsoe_tyndp, sheet_name="Line", index_col=0
    )
...
def _entsoe_ntcs(locations, tyndp_scenarios, scenario, grid, year, ntc_limit):
    ...
    # Capacity is in MW
    tyndp_import = _split_links_in_index(
        sliced_scenarios[sliced_scenarios.Parameter == "Import Capacity"]
    )
    tyndp_export = _split_links_in_index(
        sliced_scenarios[sliced_scenarios.Parameter == "Export Capacity"]
    )
