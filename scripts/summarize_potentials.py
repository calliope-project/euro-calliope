...
def summarize_potentials(
    ...
    for specific locations, the globally defined potential is used. If no globally defined potential exists, NaN is
    used.
    ...
    # Can be extended with other per-tech constraint variable of the model (pay attention to units/scaling factors).
    ...
    summary = xr.DataArray(summary, dims=("techs", "potentials", "locs", "value_unit"),
                           coords={"techs": list_of_techs, "potentials": list_of_potentials, "locs": list_of_locs,
    ...
    for potential in list_of_potentials:
        ...
        for tech in list_of_techs:
            for loc in list_of_locs:
                try:
                    summary.loc[dict(techs=tech, locs=loc, potentials=potential, value_unit="value")] = aux.loc[
                        dict(techs=tech, locs=loc)] / considered_potentials[potential]["sf"]
                    summary.loc[dict(techs=tech, locs=loc, potentials=potential,
                                     value_unit="unit")] = considered_potentials[potential]["unit"]
                except KeyError:
