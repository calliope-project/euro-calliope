...
def estimate_missing_storage_capacity_values(stations):
    ...
    storage_capacity_MWh = stations.storage_capacity_MWh.where(
        pd.notnull, other=based_on_country_specific
...
def scale_phs_acording_to_geth(stations, path_to_geth_national_capacities):
    ...
    geth_national_capacities = read_national_phs_storage_capacities(
        path_to_geth_national_capacities
...
def ensure_stations_within_basins(stations, path_to_basins, buffer_size_m):
    ...
    for station_id, station in ill_placed_stations.iterrows():
        geo_stations.loc[station_id, "geometry"] = new_coords(
            station, buffer_size_m, hydrobasins
        )
...
if __name__ == "__main__":
    preprocess_stations(
        ...
        country_codes=[
            pycountry.countries.lookup(country).alpha_3
