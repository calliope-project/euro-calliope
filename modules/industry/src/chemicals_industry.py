...
def get_chem_demand_df(
    ...
    demand = energy_df.xs("demand").sum(
        level=["section", "subsection", "country_code", "cat_name", "unit"]
    )
    ...
    # Process data
    # -------------------------------------------------------------------------
    chemicals_energy_consumption = process_chemicals_energy_consumption(
        electrical_consumption, prod_df, demand
    )
    # -------------------------------------------------------------------------
...
def process_chemicals_energy_consumption(electrical_consumption, prod_df, demand):
    ...
    chem_electricity_consumption = (
        electrical_consumption.drop("Low enthalpy heat", level="subsection")
    ...
    basic_chemicals_mass = prod_df.xs("Basic chemicals (kt ethylene eq.)").sum(
        level=["country_code"]
    )
    ...
    chem_h2_consumption = sum(masses[k] * h2_demand[k] * H2_LHV_KTOE for k in h2_demand)
    chem_methanol_consumption = sum(
        masses[k] * methanol_demand[k] * METHANOL_LHV_KTOE for k in methanol_demand
    )
    chem_co2_consumption = sum(masses[k] * co2_demand[k] for k in co2_demand)
    chem_energy_consumption = sum(
        masses[k] * energy_demand[k] * MWH_PER_T_TO_KTOE_PER_KT for k in energy_demand
    )
    # Space heat
    space_heat_demand = (
    ...
    chem_consumption = pd.concat(
        [
            chem_electricity_consumption.add(chem_energy_consumption).droplevel(
                "carrier"
            ),
            chem_methanol_consumption.assign(unit="ktoe").set_index(
                "unit", append=True
            ),
            chem_h2_consumption.assign(unit="ktoe").set_index("unit", append=True),
            chem_co2_consumption.mul(1e3)
            .assign(unit="t")
            .set_index("unit", append=True),
            space_heat_demand,
        ],
        names=["carrier"],
        keys=["electricity", "methanol", "hydrogen", "co2", "space_heat"],
    )

    chem_consumption = chem_consumption.assign(cat_name="Chemicals Industry").set_index(
        "cat_name", append=True
    )
