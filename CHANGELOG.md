...
* **UPDATED** Computation of electricity load timeseries:
    ...
    * A .csv file with all European industrial installations and their respective locations, industrial activities, and emissions is included in the repository (ca. 68 000 installation, year of data: 2020), but can also be re-generated from the databases using the rules provided in industrial-emissions.smk.
    * When comparing the new and previous timeseries (as generated by the default workflow) for each timestep and in each unit, a median relative deviation of 0.13 is observed for the regional resolution. No changes occur for the national and continental resolution.
