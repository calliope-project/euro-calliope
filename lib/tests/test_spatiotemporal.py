...
@pytest.fixture
def single_shape():
    return gpd.GeoDataFrame(geometry=[box(0, 0, 4, 4)], crs=WGS84)


@pytest.fixture
def single_shape_small():
    return gpd.GeoDataFrame(geometry=[box(0, 0, 3, 4)], crs=WGS84)
...
def make_spatiotemporal_data(make_timeseries):
    def _make_spatiotemporal_data(values):
        ...
        da = (
            pd
            .concat([
                make_timeseries(values[0]).to_frame().assign(lon=1, lat=1),
                make_timeseries(values[1]).to_frame().assign(lon=1, lat=3),
                make_timeseries(values[2]).to_frame().assign(lon=3, lat=3),
                make_timeseries(values[3]).to_frame().assign(lon=3, lat=1)
            ])
            .reset_index()
            .set_index(["lat", "lon", "timestep"])
            .to_xarray()
            ["value"]
        )
        da.attrs["crs"] = WGS84
...
def non_rectangular_spatiotemporal_data(make_timeseries):
    da = (
        pd
        .concat([
            make_timeseries(8).to_frame().assign(lat=1, lon=1),
            make_timeseries(8).to_frame().assign(lat=1, lon=3),
            make_timeseries(8).to_frame().assign(lat=3, lon=1),
            make_timeseries(8).to_frame().assign(lat=3, lon=3),
            make_timeseries(8).to_frame().assign(lat=5, lon=3)
        ])
        .reset_index()
        .set_index(["lat", "lon", "timestep"])
        .to_xarray()
...
def spatiotemporal_data_with_nodata(make_timeseries):
    da = (
        pd
        .concat([
            make_timeseries(3).to_frame().assign(lat=1, lon=1),
            make_timeseries(9).to_frame().assign(lat=1, lon=3),
            make_timeseries(9).to_frame().assign(lat=3, lon=1)
        ])
        .reset_index()
        .set_index(["lat", "lon", "timestep"])
        .to_xarray()
...
def test_fails_with_wrong_crs_in_shapes(single_shape, any_spatiotemporal_data):
    single_shape = single_shape.to_crs(EPSG3035)
    with pytest.raises(AssertionError):
        area_weighted_time_series(shapes=single_shape, spatiotemporal=any_spatiotemporal_data)
...
def test_fails_with_wrong_crs_in_spatiotemporal(single_shape, any_spatiotemporal_data):
    any_spatiotemporal_data.attrs["crs"] = EPSG3035
    with pytest.raises(AssertionError):
        area_weighted_time_series(shapes=single_shape, spatiotemporal=any_spatiotemporal_data)
