...
class TestLoadHelperFunctions:
    ...
    def test_filter_outliers(
        self, min_thresh, max_tresh, expected_foo, expected_bar, foobar_df
    ):
        ...
        config = {
            "outlier-data-thresholds": {
                "relative-to-mean-min": min_thresh,
                "relative-to-mean-max": max_tresh,
            }
        }
    ...
    def test_interpolate_gaps(
        self, interpolate_timesteps, expected_foo, expected_bar, foobar_df
    ):
        input_load = foobar_df(
            foo=[np.nan, 0, 1, 2, np.nan], bar=[0, np.nan, np.nan, np.nan, 4]
        )
    ...
    def test_ignore_feb_29th(
        self, model_year, next_available_year_of_data, input_timeseries, expected
    ):
    ...
    def test_countries_with_missing_data_in_model_year(
        self, input_foo, input_bar, expected, foobar_df
    ):
...
class TestLoadDummyData:
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            [
                {"acceptable_year_diff_for_gap_filling": 1},
                r"(?!=.*ALB)(?=.*DEU)",
                "2016-02-27",
            ],
            [{"fill_29th_feb_from_28th": False}, r"(?=.*ALB)(?!=.*DEU)", "2016-02-29"],
            [
                {"data_source_priority_order": ["foo"]},
                r"(?!=.*ALB)(?=.*DEU)",
                "2016-02-27",
            ],
        ),
