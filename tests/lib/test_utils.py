...
class TestRenameAndGroupby:
    ...
    @pytest.mark.parametrize(
        ("rename_dict", "expected"), [
            ({"A": "A1", "B": "B1", "C": "C1", "D": "D1", "E": "E1"},
             {"A1": [1, 5], "B1": [np.nan, 4], "C1": [np.nan, np.nan], "D1": [4, np.nan], "E1": [5, 1]}),
            ({"A": "A1", "B": "A1", "C": "A1", "D": "A1", "E": "A1"},
             {"A1": [10, 10]}),
            ({"A": "A1", "B": "A1", "C": "B1", "D": "B1", "E": "E1"},
             {"A1": [1, 9], "B1": [4, np.nan], "E1": [5, 1]}),
    ...
    def test_rename_dim(self, da):
        ...
        assert new_da.coords["baz"].to_index().symmetric_difference(expected.keys()).empty
        for dim_item, da_data in expected.items():
...
class TestMergeDA:
    ...
    def test_merge_da_extend_two_dims_with_three_das(self, da1, da2, da3):
        ...
        self.assertions(new_da, [da1, da2, da3], ["X", "Y", "Z"], ["A", "B", "C", "D"], 14)
