    ...
    def _make_timeseries(value):
        return pd.Series(
            index=pd.period_range(
                start="2021-04-18 12:00", periods=2, freq="D", name="timestep"
            ),
            data=value,
            name="value",
        )
...
@pytest.mark.parametrize(
    "crs1,crs2", [(EPSG3035, EPSG27700), (WGS84, EPSG3035), (WGS84, EPSG27700)]
)
...
@pytest.mark.parametrize(
    "correct_name,wrong_name", [("y", "lat"), ("x", "lon"), ("timestep", "timesteps")]
)
...
def test_handles_non_rectangular_data(
    single_shape, non_rectangular_spatiotemporal_data
):
    weighted_ts = area_weighted_time_series(
        shapes=single_shape,
...
def test_handles_not_too_many_nans_within_shapes(
    single_shape, make_spatiotemporal_data
):
    spatiotemporal_data = make_spatiotemporal_data([0, np.nan, 2, 3])
    area_weighted_time_series(
        shapes=single_shape,
...
def test_fails_with_too_much_nodata_within_shapes(
...
def test_handles_not_too_much_nodata_within_shapes(
    single_shape, spatiotemporal_data_with_nodata_within_single_shape
):
    area_weighted_time_series(
        shapes=single_shape,
...
def test_partial_match(single_shape_small, make_spatiotemporal_data):
    ...
    assert (
        pytest.approx(expected_mean, rel=0.001) == weighted_ts.iloc[:, 0].values.mean()
    )


def test_handles_reverted_coordinates_gracefully(
    single_shape, reverted_spatiotemporal_data
):
    area_weighted_time_series(
        shapes=single_shape,
