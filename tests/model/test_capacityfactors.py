...
def test_capacity_factors_are_floored(capacity_factor_timeseries, config):
    ...
    assert (
        capacity_factor_timeseries[capacity_factor_timeseries > 0].min().min() >= floor
    )
...
def test_open_field_vs_rooftop(
    open_field_pv_capacity_factor_timeseries,
    rooftop_pv_capacity_factor_timeseries,
    location,
):
...
def test_wind_onshore_exists_at_every_location(
    wind_onshore_capacity_factor_timeseries, location
):
    assert wind_onshore_capacity_factor_timeseries.loc[:, location].mean() > 0


def test_open_field_pv_exists_at_every_location(
    open_field_pv_capacity_factor_timeseries, location
):
    assert open_field_pv_capacity_factor_timeseries.loc[:, location].mean() > 0


def test_rooftop_pv_exists_at_every_location(
